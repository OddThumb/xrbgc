...
if (is.null(opt$primary) | is.null(opt$label)){
  print_help(opt_parser)
  stop("At least two argument must be supplied (input file, label)", call.=FALSE)
}
...
test.data.norm[,Norm_cols] <- scale(as.data.frame.matrix(test.data.norm)[,Norm_cols],
                                    center=attr(scaled_obj, "scaled:center"), # Same parameter as training data
                                    scale=attr(scaled_obj, "scaled:scale"))   # Same parameter as training data
if ( eval.exist ) {
...
write.table(sortedScore, file = paste(WD_FI, 'sortedScore.txt', sep='/'), quote=FALSE, sep='\t\t\t')
