...
ecdf.plot <- function(data.list) {
  ...
  for (fn in feature.names) {
    ...
    ggecdf(data = plotData, x = fn, color='GC', title = paste(fn," CDF (",suffix,")",sep=''), orientation = 'reverse') + 
      grids(linetype = "dashed") + 
      labs(y='Cumulative fraction') +
      theme(legend.position = "top") +
      guides(colour = guide_legend(title = "Globular Cluster", title.position = "top")) +
      font("title", size = 14, color = "black", face = "bold")
    ggsave(filename = paste(WD_DA,'/cdf_',fn,'.pdf',sep = ''), device = 'pdf', width = 5, height = 4)
  }
}
...
ens.predict <- function(caretmodels, stack.list, datalist) {
  ret.list <- list()
  for (i in seq_along(datalist)) {
    probs <- lapply(caretmodels, predict, newdata=datalist[[i]], type="prob")
    probs <- lapply(probs, function(x) x[,target]) %>% as.data.frame()
    
    for (j in seq_along(stack.list)) {
      stack.name <- names(stack.list)[j]
      stk_probs <- predict(stack.list[[j]], newdata=datalist[[i]], type='prob')
      probs[,stack.name] <- stk_probs
    }
...
# p-values from correlation matrix
...
# Accuracy Curve plot
ggAccCurve <- function(data, method_name=NULL, col="id", linetype="id", height = 7, width = 5, eachModel=FALSE) {
  if ( eachModel ) {
    p <<- ggplot(data, aes_string(x="x", y="acc", col=col, linetype=linetype)) +
      geom_line() + 
      geom_point() + 
      geom_errorbar(aes(ymin = acc - se, ymax = acc + se), width=0.5) +
      ggtitle(paste("Accuracy curve for ", target, "(each model)")) +
      facet_wrap(~id, ncol = 2, scales = 'free') +
      scale_x_continuous(breaks = seq(2), labels = seq(2)) +
      xlab('fset') + ylab('Accuracy (%)') +
      theme_pubr() + grids(linetype = "dashed") + expand_limits(y=100) +
      labs(col=NULL, linetype=NULL)
    ggsave(filename = paste(WD_ACC,"/AccCurve_eachModel.pdf", sep = ""), plot = p,
           device = "pdf", height = height, width = width)
    p
  } else {
    p <<- ggplot(data, aes_string(x="x", y="acc", col=col, linetype=linetype)) +
      geom_line() + geom_point() + geom_errorbar(aes(ymin = acc - se, ymax = acc + se), width=0.5) +
      ggtitle(paste("Accuracy curve for ", target, " (", method_name,")")) +
      scale_x_continuous(breaks = seq(2), labels = seq(2)) +
      xlab('fset') + ylab('Accuracy (%)') +
      theme_pubr() + grids(linetype = "dashed") + expand_limits(y=100) +
      labs(col=NULL, linetype=NULL)
    ggsave(filename = paste(WD_ACC,"/AccCurve_",method_name,".pdf", sep = ""), plot = p,
           device = "pdf", height = height, width = width)
    p
  }
}
...
# Input data
trainset <- train.data.norm %>% select(-GC) %>% binarize() %>% `rownames<-`(NULL) %>% randomize()
testset  <- test.data.norm  %>% select(-GC) %>% binarize() %>% `rownames<-`(NULL) %>% randomize()
if ( eval.exist ) {
  evalset  <- eval.data.norm %>% select(-GC) %>% binarize() %>% `rownames<-`(NULL) %>% randomize()
}
...
if ( eval.exist ) {
  ecdf.plot( data.list = list(train.data.norm, test.data.norm, eval.data.norm) )
} else {
  ecdf.plot( list(train.data.norm, test.data.norm) )
}
...
# Data analysis directory
WD_FI <- paste(WD,"3.FeatureImportance", sep='/')
dir.create(WD_FI)
...
Control <- trainControl(method="repeatedcv", number=10, repeats=5,
                        returnResamp = 'final', verboseIter = TRUE, classProbs=TRUE,
                        allowParallel = TRUE)
caretmodels <- caretList(class~., data=trainset, trControl=Control, methodList=selected.models)
ensem <- caretEnsemble(caretmodels, metric="Accuracy", trControl=Control)
score <- varImp(ensem)[,'overall', drop=F]
sortedScore <- score %>% arrange(-overall)
print(sortedScore)


# Saving sorted feature importance scores
save(sortedScore, file = paste(WD_FI, 'sortedScore.Rdata', sep='/'))
write.table(sortedScore, file = paste(WD_FI, 'sortedScore.txt', sep='/'))
cat("\n>>> Importance scores are saved in ",WD_FI)

# Plotting 3D plot with top3 features
x.axis <- rownames(sortedScore)[1] %>% as.character()
y.axis <- rownames(sortedScore)[2] %>% as.character()
z.axis <- rownames(sortedScore)[3] %>% as.character()
...
# Operating ML
each.iter <- 20
fno <- nrow(sortedScore)
cl <- makeCluster(nproc)
registerDoSNOW(cl)
for (i in seq(fno)) {
  feature_select <- rownames(sortedScore)[c(1:i)]
  ...
  for (iter in seq(each.iter)){
    ...
    caretmodels <- caretList(class~., data=traindata, trControl=Control, methodList=selected.models)
    
    cat('\n   Training for stacking (glm)...')
    stack.glm <- caretStack(caretmodels, method="glm", metric="Accuracy", trControl=Control)
    cat('\n   Training for stacking (rf)...')
    stack.rf  <- caretStack(caretmodels, method="rf", metric="Accuracy", trControl=Control)
    cat('\n   Training for stacking (nb)...')
    stack.nb  <- caretStack(caretmodels, method="nb", metric="Accuracy", trControl=Control)
    stack.list <- list('glm.stack'=stack.glm, 'rf.stack'=stack.rf, 'nb.stack'=stack.nb)
    
    prob.each <- ens.predict(caretmodels, stack.list, datalist = data.list)
    pred.each <- lapply(prob.each, function(x) as.data.frame(cbind(apply(x, 2, function(y) ifelse(y >= 0.5, 1, 0)))) )
    
    for (nm in names(pred.each)) {
      onezeros <- apply(pred.each[[nm]], 2, factor, levels = c(1,0))
      confMat <- onezeros %>% 
        apply(2, function(x) confusionMatrix(as.factor(x),
                                             reference = factor(ifelse(data.list[[nm]]$class == target, 1, 0),
                                                                levels = c(1,0))))
      accuracy.list[[nm]] <- rbind(accuracy.list[[nm]],
                                   confMat %>% lapply(function(x) x$overall[1]) %>% bind_cols() %>% mutate(x=i))
    }
  ...
  prob.feat[[i]] <- prob.aggr
  pred.feat[[i]] <- pred.aggr
}
registerDoSEQ()
stopCluster(cl)
...
stack.name <- c('glm.stack','rf.stack','nb.stack')
stack.data <- acc.melt %>% filter(variable %in% stack.name)
major.data <- acc.melt %>% filter(variable == 'mjv')
ensem.data <- bind_rows(stack.data, major.data)


# Plotting Accuracy curves
ggAccCurve(eachModel.data, col="variable", linetype="variable", height=18, width=8, eachModel=TRUE)
ggAccCurve(major.data, method_name = "majority_voting", col="id", linetype="id")
ggAccCurve(ensem.data, method_name = "all_ensembles", col="variable", linetype="variable")


# save accuracy results
train.acc.result <- major.data %>% filter(id=='train')
test.acc.result  <- major.data %>% filter(id=='test')
write.table(train.acc.result, file = paste(WD_ACC,"train_acc_result.txt",sep='/'))
write.table(test.acc.result, file = paste(WD_ACC,"test_acc_result.txt",sep='/'))
if ( eval.exist ) {
  ALL.acc.result  <- major.data %>% filter(id=='eval')
  write.table(ALL.acc.result, file = paste(WD_ACC,"eval_acc_result.txt",sep='/'))
}


# ==================== ROC curves for Each Feature Subset ======================
# Majority Voting
mjv.probs <- prob.feat %>% lapply(function(x) lapply(x, function(y) y %>% as.data.frame() %>% select(-names(stack.list)) %>% apply(1, mean)))


# Stacking
stk.probs <- prob.feat %>% lapply(function(x) lapply(x, function(y) y[,names(stack.list)]))
...
for (f in seq(fno)) {
  for(m in names(stack.list)) {
    # Plot ROC curve
    if ( eval.exist ) {
      pdf(file = paste(WD_ROC,"/",m,"_ROC_F",f,".pdf", sep = ""), height = 3, width = 8)
      par(mfrow=c(1,3))
      PlotROC(ref=Train_y, pred=stk.probs, set='train', m=m)
      PlotROC(ref=Test_y,  pred=stk.probs, set='test',  m=m)
      PlotROC(ref=Eval_y,  pred=stk.probs, set='eval',  m=m)
      dev.off() 
    } else {
      pdf(file = paste(WD_ROC,"/",m,"_ROC_F",f,".pdf", sep = ""), height = 3, width = 6)
      par(mfrow=c(1,2))
      PlotROC(ref=Train_y, pred=stk.probs, set='train', m=m)
      PlotROC(ref=Test_y,  pred=stk.probs, set='test',  m=m)
      dev.off() 
